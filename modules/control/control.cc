...
Status Control::Init() {
  ...
  AdapterManager::Init(FLAGS_control_adapter_config_filename);//读message消息类别
  ...
  //$)AKx6(#,7@V93uJ<;/35A>N4W<18:C#,O{O"4%7"RQ>-5=@4
  // come
...
Status Control::Start() {
  ...
  AINFO << "Control default driving action is "//默认驱动模式：
...
Status Control::ProduceControlCommand(ControlCommand *control_command) {
  ...
  if (!status.ok()) {
    AERROR_EVERY(100) << "Control input data failed: "
                      << status.error_message();//Error发布频率100hz
  ...
  // if planning set estop, then no control process triggered$)AHg9{9f;.M#V9#,Tr2;;a4%7"HN:N?XVF9}3L
  if (!estop_) {
